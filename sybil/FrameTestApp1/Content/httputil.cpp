...
BSTR GuidToString(UUID& uuid)
{
	WCHAR cb[256];	
	swprintf_s(cb,256, L"%.8x-%.4x-%.4x-%.2x%.2x-%.2x%.2x%.2x%.2x%.2x%.2x", uuid.Data1, uuid.Data2, uuid.Data3, uuid.Data4[0],uuid.Data4[1],
			uuid.Data4[2],uuid.Data4[3],uuid.Data4[4],uuid.Data4[5],uuid.Data4[6],uuid.Data4[7] );	

	return ::SysAllocString(cb);
}
...
{
	UINT cnt = m.size();
	BSTR* ar = new BSTR[cnt];

	auto it = m.begin();
	for( UINT i = 0; i < cnt; i++ )
	{
		std::wstring v = it->first;
		v += L": ";
		v += it->second;

		ar[i] = ::SysAllocString( v.c_str());
	}
