...
static IBinary _hash( BYTE* p, size_t plen, int typ )
{
	BYTE* ret = nullptr;
	...
	BCryptHashData(hHash, p, (ULONG)plen, 0);
...
bool aes128ex( bool IsEncrypt, byte* pwd, size_t pwdlen, IStream* src, IStream** dst )
{
	...
	status = BCryptGenerateSymmetricKey(hAlgorithm, &hKey, pKeyObject, dwKeyObjectSize, pwd, (ULONG)pwdlen,0 ); // KeyLength.dwMaxLength, 0); // KeyLength.dwMaxLength,0);
	...
	if (IsEncrypt)
	{
		...
		DWORD  dwEncryptDataSize = (((DWORD)dwDataSize - 1) / ENCRYPT_BLOCK_SIZE + 1) * ENCRYPT_BLOCK_SIZE;
		LPBYTE lpEncryptData = new byte[dwEncryptDataSize];
				
		status = BCryptEncrypt(hKey, lpData, (ULONG)dwDataSize, NULL, pIV, IVLength, lpEncryptData, (ULONG)dwEncryptDataSize, &dwEncryptDataSize, BCRYPT_BLOCK_PADDING|BCRYPT_PAD_PKCS1);
	...
	{			
		...
		LPBYTE lpData;
		size_t  dwDataSize;
		DWORD dwDecryptDataSize;

		IBinary bins = ToIBinary(src);
		lpEncryptData = IBinaryPtr(bins);
		dwEncryptDataSize = (DWORD)IBinaryLen(bins);
		...
		status = BCryptDecrypt(hKey, lpEncryptData, dwEncryptDataSize, NULL, pIV, IVLength, lpData, (ULONG)dwDataSize, &dwDecryptDataSize, BCRYPT_BLOCK_PADDING| BCRYPT_PAD_PKCS1);
...
static BSTR HexString( BYTE* p, size_t len )
{
	std::wstring rval;
...
{	
	...
	StrToUtf8( text, (int)wcslen(text), &s1 ); 
	Hash( s1, 0, &d1 );
...
{
	int cblen = ::WideCharToMultiByte( CP_UTF8, 0, str,(int)wcslen(str), 0,0,0,0);
	
	byte* cb = new byte[cblen+1];

	::WideCharToMultiByte( CP_UTF8, 0, str,(int)wcslen(str), (LPSTR)cb,cblen,0,0);
...
{
	int cblen = ::MultiByteToWideChar( CP_UTF8,0, (LPSTR)IBinaryPtr(utf8), (int)IBinaryLen(utf8), nullptr,0);
	BSTR r = ::SysAllocStringLen(NULL,cblen);
	cblen = ::MultiByteToWideChar( CP_UTF8,0, (LPSTR)IBinaryPtr(utf8), (int)IBinaryLen(utf8), r, cblen);
	*ret = r;
	return (cblen >= 0);
}
bool AsciiToBSTR( const IBinary& asc, BSTR* ret )
{
	int cblen = ::MultiByteToWideChar( CP_ACP,0, (LPSTR)IBinaryPtr(asc), (int)IBinaryLen(asc), nullptr,0);
	BSTR r = ::SysAllocStringLen(NULL,cblen);
	cblen = ::MultiByteToWideChar( CP_ACP,0, (LPSTR)IBinaryPtr(asc), (int)IBinaryLen(asc), r, cblen);
	*ret = r;
	return (cblen >= 0);
}
...
bool WriteFile( LPCWSTR fnm, const IBinary& src, int typ )
{
	...
	if ( INVALID_HANDLE_VALUE != h )
	{
		...
		_ASSERT(IBinaryLen(src) < (size_t)0xFFFFFFFF );
		::WriteFile(h, IBinaryPtr(src), (DWORD)IBinaryLen(src), &dw, nullptr );
		::CloseHandle(h);
		return true;
	}
	return false;
...
IStream* ToIStream( const IBinary& src )
{
	...
	DWORD dw;
	is->Write(IBinaryPtr(src), (ULONG)IBinaryLen(src), &dw );
...
IBinary HMACSHA256(const IBinary& _key, const IBinary& _data )
{
	...
	size_t keySize = IBinaryLen(_key);
	const BYTE* message = IBinaryPtr(_data);
	size_t messageSize = IBinaryLen(_data);
	...
	//create a hash
	if (0!=(status = BCryptCreateHash(hAlg,&hHash,pbHashObject,(ULONG)cbHashObject,(PBYTE)key, (ULONG)keySize,	0)))
	{
		error = true;
		goto Cleanup;
	}
	...
	//hash some data
	if (0!=(status = BCryptHashData(hHash,(PBYTE)message,(ULONG)messageSize,0)))
	{
		error = true;
		goto Cleanup;
	}
...
{
	auto ar = Split(headers_CRLF, L"\r\n");

	std::map<std::wstring, std::wstring> ret;
	for( auto& it : ar )
	{
		auto ar2 = Split(it.c_str(), L":");
