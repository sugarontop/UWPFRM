...
FRectFBoxModel D2DTextbox::Rect1(const FRectF& rc)
{
	FRectFBoxModel ret(rc);
	ret.Padding_.l = 2;
	ret.Padding_.t = 2;
	return ret;
}
...
void D2DTextbox::OnTextUpdated()
{
	...
	UINT32 c;
	if (HR(layout_->GetLineMetrics(&lm,1,&c)))
	...
	switch( message )
	{
		case WM_PAINT:
		{
			...
			else if ( ti_.decoration_start_pos != ti_.decoration_end_pos )
			{
				...
				const FRectF* rcc = ti_.rcChar().get(); 
		...
		case WM_MOUSEMOVE :
		{
			...
			if (this == GetParentControl()->GetCapture() && bMouseSelectMode_ )
			{
				...
				if ( !bl )
				{
					if ( 0 < ti_.rcChar().count() && ti_.sel_start_pos < ti_.sel_end_pos ) //左から右へ 終端
						xi = ti_.rcChar().count();
					else
...
{	
	...
	{		
		...
		if ( xrc.count() > ti_.sel_start_pos && ti_.line_cnt > 1 )
		{
			FRectF rc3 = xrc.get(ti_.sel_start_pos);

			rc3.left=0;

			rc3.Offset(rc2.left, rc2.top);

			ti_.rcIme = mat_.LPtoDP(rc3); 
		}
		else
			ti_.rcIme = mat_.LPtoDP(rc2);

		//ti_.rcIme = mat_.LPtoDP(rc2);
		ti_.rcTextbox = mat_.LPtoDP(rc);  // device coordinate
...
static int RCfromPosition( V4::D2CoreTextBridge& br, int pos, int* prow, int* pcol )
{
	//FRectF* rc = br.info_->rcChar().rcChar_.get();
	const FRectF* rc = br.info_->rcChar().get();

	int rccnt = br.info_->rcChar().count();
...
static bool RCToPosition( D2CoreTextBridge& br, int target_row, int target_col, int* ppos )
{
	...
	//FRectF* rc = br.info_->rcChar().rcChar_.get();
	const FRectF* rc = br.info_->rcChar().get();
	const size_t len = br.info_->text.length();
